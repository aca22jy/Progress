...
class BERT_multilabel(torch.nn.Module):
    def __init__(self):
        ...
        # 如果flash_attn可用，启用flash_attention_2
        config = None
        if 'has_flash_attn' in globals() and has_flash_attn:
            config = AutoConfig.from_pretrained(args.bert_model)
            config.attn_implementation = "flash_attention_2"
            
        # 使用配置加载模型
        if config:
            self.l1 = AutoModel.from_pretrained(args.bert_model, config=config)
        else:
